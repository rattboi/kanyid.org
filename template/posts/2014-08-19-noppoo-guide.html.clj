...
(h2 "Introduction")
(p "Well, I finally did it; I bought my first mechanical keyboard, a " (link "Noppoo Choc Mini 2M" "http://www.amazon.com/dp/B00FVTQ1OE/") " with " (link "Cherry MX Blue" "http://deskthority.net/wiki/Cherry_MX_Blue") " switches from " (link "Massdrop" "https://massdrop.com") ". As this is my first mechanical keyboard, I really didn't know what to expect. I knew that there would be a few function keys to control some of the features, but I didn't really take the time to read up on them, past noting that there was no way through hardware to " (link "rebind the Capslock key to Escape" "http://vim.wikia.com/wiki/Avoid_the_escape_key") ". After looking at many keyboards and failing to find this feature, I've made my peace with it. But I digress...")

(img {:style "width: 75%; display: block; margin-left: auto; margin-right: auto"} "Image of Noppoo Choc Mini 2M keyboard" "/img/noppoo.jpg")
...
(h2 "The Problem")
(p "After briefly testing and verifying the basic keypress functionality was working, I started looking into what features the keyboard offered. I searched online for a manual, and was suprised to find that there isn't one, at least not one that I could find. It seems a little strange to me that Noppoo doesn't even list this product on their website. After a little more research, I was also surprised to find that the " (link "Deskthority wiki" "http://deskthority.net/wiki/Noppoo_Choc_Mini") " does not cover all the features of the Choc Mini. So I was left searching for each thing individually, finding bits of information and piecing it together. That was annoying, so I'm compiling the info and placing it here.")

(h2 "Media Keys (Left Hand Cluster)")
(p "There were certain features of the keyboard that were immediately apparent, like how to use the media keys. There are laser-etched graphics on the top of each key that shows its primary function, and also screenprinted graphics on the front side of the key showing any secondary use. The media keys are in a cluster on the left hand, under the asdf keys, with volume control under the qwe keys. Fn+A/S/D/F controls playback, Fn+Q/W/E controls volume. Strangely, within the media key cluster is also Fn+R, which disables the Windows key. This would be a great boon if you are a Windows gamer, as it would disallow accidental presses of the Windows key, which would kick you out of your game. For me, this is basically useless, as the Win key is my main way of interacting with " (link "Awesome, my window manager." "http://awesome.naquadah.org/"))

(h2 "Numpad Emulation (Right Hand Cluster)")
(p "This functionality isn't actually a chorded motion, like the Fn+<key> bindings. This is a modal setting, based on the Numlock state. If Numlock is on, which you can toggle via Fn+PrtSc, then you can use your right hand for standard Numpad functions. This should be fairly obvious, so I'm not gonna go into detail. One interesting thing is that the comma key in Numlock mode is bound to 00. I could see this as useful if you're used to financial calculators.")

(img {:style "width: 75%; display: block; margin-left: auto; margin-right: auto"} "Another image of Noppoo Choc Mini 2M keyboard" "/img/noppoo2.jpg")
...
(h2 "Hidden Extras")
(p "Last but not least, there's one set of 'hidden' Fn key mappings for controlling the brightness of the LED backlighting. Fn+Up/Down control 4 levels of brightness, from Really Bright, to Pretty Bright, to Sorta Bright, to Off. There might be other hidden features, but I haven't disovered them yet.")

(h2 "Function Key Summary")
[:b  (table ['["Key Combination" "Explanation"]
